...
describe "before create unique id generation" do
  
  before do
    @widget = WidgetBeforeCreate.create(:name => "create seo friendly id")
  end
  ...
  describe "strip out special characters, spaces, or truncate strings" do
    ...
    it "generates seo friendly id without comma" do
      widget = WidgetBeforeCreate.create(:name => "Hi There! What's up y'all")
    ...
    it "generates truncated seo friendly id with a 50 character limit on longer strings" do
      widget = WidgetBeforeCreate.create(:name => "Hi There! What's up y'all, Howdy there, whatcha doing, whatcha up to?")
    ...
    it "generates truncated seo friendly id with a 50 character limit on longer strings without spaces" do
      widget = WidgetBeforeCreate.create(:name => "onecontinuouslylongwordalskdjflaskjfdlaskdfjlkjsdjsdflkjasdfljlaskdjfallskdfajsdkfj")
  ...
  describe "seo friendly ids are unique" do
    it "generates unique id from same passed in string" do
      widget = WidgetBeforeCreate.create(:name => "this is a test")
  ...
  describe "seo friendly to_param" do    
    it {expect(WidgetBeforeCreate.new.to_param).to be_nil}
    text_name = "Test this string for unique Iding!"
    
    widget = WidgetBeforeCreate.create!(:name => text_name)
    it {expect(widget.to_param).to eq("test-this-string-for-unique-iding")}
  ...
  describe "seo friendly collision with original id" do
    before do
      ...
      names.each do |name|
        WidgetBeforeCreate.create!(:name => name)
      end
    end
    
    it "seo friendly no collisions" do
      uids = WidgetBeforeCreate.all.collect! {|w| w.unique_id}
